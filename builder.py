...
def build_and_parse(task_description, original_prompt):
    ...
    """
    print(f"[Builder] Generative new skill for: {task_description}")
    ...
    build_prompt = (
        ...
        "Requirements:\n"
        "1. It MUST have an `execute(**kwargs)` function.\n"
        "2. It MUST be a valid Python file.\n"
        "3. Do NOT use any external libraries unless standard or 'ollama'.\n"
        "4. The FIRST line of the file MUST be a comment `# filename: <name_of_file>.py`.\n"
        "5. Output ONLY the code, no markdown backticks, no explanations."
    )

    messages = [
        {'role': 'system', 'content': 'You are a Python coding assistant. Write clean, working code.'},
        {'role': 'user', 'content': build_prompt}
    ]

    print("[Builder] Asking Qwen to write code...")
    response = ollama.chat(model=MODEL_BUILDER, messages=messages)
    content = response['message']['content']
    
    # Clean up content (strip markdown if model ignores instruction)
    ...
    if filename_match:
        ...
        # Try to guess a better name if possible or just warn
        print(f"[Builder] Warning: No filename comment found. Using {filename}")
    ...
    # We append the user's prompt and ask to extract args for the code just written
    parse_prompt = (
        f"Based on the code you just wrote, extract the arguments needed to run `execute` "
        f"from this user prompt: \"{original_prompt}\".\n"
        "Return the result as a raw JSON object `{\"arg_name\": value, ...}`.\n"
    ...
    print("[Builder] Asking Qwen to parse arguments...")
...
def parse_only(tool_name, user_prompt):
    ...
    """
    # Load the tool code to give context
    try:
